...
| `ov_portfolios` | `{portfolios: [{id,name,avatar?}], activeId}` — the multi-portfolio index (see §16). `avatar` is an emoji (defaults to `👻` / `DEFAULT_AVATAR`); absent on records written before the field existed. Created on first run from the legacy single `ov_holdings` list, which becomes the `main` portfolio; `ov_holdings` is **never** moved or deleted, so a downgrade still finds the user's original holdings. Each non-main portfolio's holdings live under `ov_holdings__<id>`. Switching/creating/duplicating/deleting repoints `holdings.ts` at the active key via `setActiveHoldingsKey()`. **Both wallet views use this** — the phone embed via `PortfolioSwitcher`, the browser via `WalletViewWeb`'s header name-menu + Edit-portfolio modal. | `createPortfolio()` / `duplicatePortfolio()` / `renamePortfolio()` / `updatePortfolio()` (name+avatar) / `deletePortfolio()` / `setActivePortfolio()` in `app/home/wallet/portfolios.ts` | `ensurePortfolios()` / `loadPortfolios()` / `getActiveId()` (same file) |
...
| `/api/market/klines` | `app/api/market/klines/route.ts` | GET `?symbol=BTC&range=7d` → `{points: [{t, close}]}`. Keyless Binance klines for the wallet's All-time-profit "BTC trend" line; symbol allow-list (BTC/ETH) only. POST `{range, assets: [{symbol, asset_type}]}` → `{series: {SYM: [{t, close}]}}` — per-holding history for the wallet History chart: crypto via Binance klines, stock/metal via Yahoo chart API (metals through the same `GC=F`-style futures map as the prices route), currency via Frankfurter daily rates (1/rate). Symbols regex-validated, ≤30 per call, per-`type:symbol:range` in-memory cache (5 min, non-empty results only); a failed upstream returns `[]` for that symbol, never an error. |
...
| `web/app/home/wallet/WalletViewWeb.tsx` | `'use client'` — **the browser wallet**: a **portfolio header** (avatar + name menu, total + hide-value eye, change line) above the history chart + donut allocation, then the Best/Worst/profit stat cards, then the asset table. `WalletShell` renders this for the browser view; the phone-app embed gets `WalletView` instead. **Header**: `PortfolioNameMenu` (avatar emoji + name `▾`) opens a menu — a **Switch** list of portfolios (when >1) plus **Edit / Duplicate / Remove**; **Edit** opens `EditPortfolioModal` (emoji-avatar picker + rename, 24-char cap). Actions row: **Add Transaction** (opens Add Asset modal) · **Create Portfolio** (`createPortfolio('New Portfolio')`) · Export/⋯ (visual-only). Portfolio state comes from `portfolios.ts`; switch/create/duplicate/delete run `reloadActive()` which re-reads holdings+snapshots for the new active key and clears prices. The change line prefers the 24h delta (`valueAgo(snapshots,24)`), falling back to all-time P&L until a day of history exists. The bottom card carries an **Assets / Transactions** toggle (`assetTab`): **Assets** is the holdings table; **Transactions** derives one **Buy** row per holding (Type · Date · Assets · Price · Amount · Fees · Notes · Actions) sorted newest-first by `purchased_at`, with `Fees = amount×avg_buy_price×fee_pct/100` and `--` fallbacks. The Transactions tab adds a filter bar (`FilterDropdown` × 2 + search): **type** (All/Buy/Sell/Transfer In/Out — only Buy/All match, since every derived row is a Buy), **coin**, and free-text over name/symbol/notes. **History chart data**: `PortfolioHistory` builds its series from market klines via `portfolioSeries.ts` (`usePortfolioHistory` POST to `/api/market/klines` per period + `buildPortfolioSeries` summing amount×close per timestamp), so 24h/7d/… show the real period's movement with a hoverable point per candle; recorded snapshots are only the fallback when every upstream fails. The header 24h change still comes from snapshots (`valueAgo`). |
| `web/app/home/wallet/WalletView.tsx` | `'use client'` — **the phone-app embed wallet only** (`?embed=wallet`): CoinMarketCap-style Portfolio (content only; the app shell supplies top/bottom bars). NOT used by the browser — see `WalletViewWeb`. Header: portfolio-name `▾` switcher (switch/create/rename/delete via `PortfolioSwitcher`), `$total` + hide-value eye + `+`, and 24h/All-time change lines. A chip row — **Holdings / All-time profit / Allocation** — swaps the whole panel below AND the asset table's third column: Holdings → value area chart + value column; All-time profit → dual-line chart (blue portfolio-profit-% + orange BTC-trend-% from `/api/market/klines`) + per-asset profit column; Allocation → donut + allocation-% column with per-row bars. Timeframe pills (24h/7d/30d/90d/All) show on Holdings + All-time profit, not Allocation. The third-column header sorts rows by that column (desc default). `+ New transaction` opens the Add Asset modal. An **Overview / Transactions** tab strip sits under the header: Overview is the chip UI above; Transactions derives one **Buy** row per holding from its `purchased_at` + `amount` + `amount×avg_buy_price`, grouped by day (newest first), with a floating `+` FAB — no separate transaction store (a full buy/sell log would need one). The Holdings value chart (`ValueChart`) builds its series from market klines via `portfolioSeries.ts` (same `usePortfolioHistory`/`buildPortfolioSeries` pipeline as `WalletViewWeb`), so every timeframe shows the period's real movement, and hover shows a time+value tooltip (`.wallet-chart-tooltip`, flipping near the right edge) plus the crosshair/dot and header-total follow. Snapshots are the fallback when upstreams fail; that fallback path (and the profit chart, still snapshot-based) trims history before the last **>20% snapshot-to-snapshot jump**: snapshots record the portfolio *total*, so adding/removing a holding (or switching portfolios) makes `recordSnapshot()` write a far-off total that would otherwise draw a vertical spike splicing two different portfolios — the chart shows only the current portfolio's run. Timeframe pills render full-width in a rounded track (`.wallet-period-row`). |
| `web/app/home/wallet/AddAssetModal.tsx` | `'use client'` — 4 category tabs, search, asset grid, amount / avg-buy-price. |
| `web/app/home/wallet/EditPortfolioModal.tsx` | `'use client'` — Edit-portfolio dialog for the browser wallet: emoji-avatar picker (`AVATARS` grid) + Portfolio-Name input (24-char cap w/ counter) + Save. Used by `WalletViewWeb` via `updatePortfolio()`. |
| `web/app/home/wallet/portfolios.ts` | Multi-portfolio index over the per-holding store (`ov_portfolios`): create/duplicate/rename/update(name+avatar)/delete/setActive, plus `holdingsKeyFor`/`snapshotsKeyFor` and the legacy-`main` migration. Used by both `WalletView` (phone) and `WalletViewWeb` (browser). |
| `web/app/home/wallet/portfolioSeries.ts` | `'use client'` — market-derived portfolio history shared by both wallet charts. `buildPortfolioSeries(holdings, seriesBySymbol, livePrices, cutoff, now, liveValue)`: grid = densest in-range symbol series; per holding, price at t = last close ≤ t (flat backfill before the first point); holdings without history contribute their live price flat; live point appended last; returns `[]` (→ snapshot fallback) when no series has ≥2 in-range points. `usePortfolioHistory(holdings, rangeKey)` POSTs `/api/market/klines`, keyed on distinct (symbol,type) pairs + range so amount edits/price polls don't refetch. Mirrored by `portfolioSeries.logic.test.mjs`. |
...
| `web/app/home/wallet/assets.ts` | Asset catalog (~200 crypto / 50 stocks / 4 metals / 10 currencies) + logo/colour/glyph resolution + shared formatters. Crypto logos come from CMC's id-keyed CDN via the pinned `CMC_LOGO_IDS` map (see **Crypto logos** below). |
...
**Crypto logos.** Same source as the chart engine's watchlist icons: CMC's id-keyed static CDN (`s2.coinmarketcap.com/static/img/coins/64x64/<id>.png`). The wallet's previous primary — CoinCap's ticker-keyed CDN — 404'd on 46 of the 203 cataloged coins (audited 2026-07-24: everything post-CoinCap-era like NEAR/HBAR/ARB/OP/APT/SUI, plus id-mismatch tickers), leaving letter chips even after the jsdelivr fallback. Unlike the engine, which resolves ids live from `/api/market/coinlogos`, the wallet's catalog is static, so `CMC_LOGO_IDS` in `assets.ts` pins all 203 ids at authoring time — a live listing mis-assigns ambiguous tickers to higher-cap claimants (TON, ALPHA, GMT) and drops rebranded projects entirely (Toncoin→Gram id 11419, Alpha Venture DAO→Stella 7232, Frax Share→FRAX 6953, Fantom 3513, Render 5690, Polygon/MATIC 3890). Every id was name-matched against CMC's full coin map and every logo URL verified 200 with a real body. An uncataloged symbol still walks the old chain: CoinCap (`COINCAP_ID_MAP` override or lowercased ticker) → jsdelivr `cryptocurrency-icons` → coloured letter chip.

**Icon colours + dark glyphs (round 2, same audit).** `getAssetColor` feeds the allocation donut, chip backgrounds and legend dots, and `BRAND_COLORS` carries several literal brand blacks (NEAR, XLM, HBAR, ALGO…) — a 100%-NEAR portfolio drew a black-on-black donut. `CRYPTO_ICON_COLORS` (generated by `gen-colors.mjs` in the same audit) holds each logo's *measured* dominant colour: bright brand entries kept verbatim, near-black brands replaced by the logo's real colour (NEAR → green `#01d88a`), genuinely monochrome logos mapped to a neutral gray rather than an invented hue. `getAssetColor` then floors lightness on whatever it resolves (`visibleColor`) so the remaining near-black brands (XRP, Nike, Palantir) stay visible without rewriting their entries. Separately, 12 logos are a dark glyph on transparency (measured: mean luminance < 60, opaque coverage < 85% — ADA, ONDO, APT, WLD, AR, BAL, CVC, TLM, OCEAN, CFX, LSK, 1INCH); `needsLightChip` backs those with a light disc in `AssetIcon`, mirroring the engine's `DARK_LOGO`/`.on-light` treatment. `AssetIcon` also unmounts its fallback letter on the image's `onLoad` — with transparent-background logos the glyph would otherwise show through permanently. Verified in a real Chromium against a dev server on :3333: 203 crypto + 50 stock + 4 metal + 10 currency tiles, 0 failed images, donut stroke `#01d88a` for a NEAR-only portfolio, 0 console errors.
