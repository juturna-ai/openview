...
| `web/app/home/wallet/WalletViewWeb.tsx` | `'use client'` — **the browser wallet**: the original stat-card layout (total + Best/Worst performer cards + history chart + donut allocation + asset table). `WalletShell` renders this for the browser view; the phone-app embed gets `WalletView` instead. Restored after the CMC redesign was mistakenly applied to the web view (the phone layout is embed-only). The bottom card carries an **Assets / Transactions** toggle (`assetTab`): **Assets** is the existing holdings table; **Transactions** derives one **Buy** row per holding (Type · Date · Assets · Price · Amount · Fees · Notes · Actions) sorted newest-first by `purchased_at`, with `Fees = amount×avg_buy_price×fee_pct/100` and `--` fallbacks for missing fee/notes — no separate transaction store (a full buy/sell log would need one). |
