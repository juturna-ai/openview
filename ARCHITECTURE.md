...
- **Non-Ethereum wallet token detail showed nothing** — clicking a wallet on any chain but Ethereum opened an empty detail panel. `getTokens` looks the EVM chain up in `BLOCKSCOUT_HOSTS` and, with no host, returns an empty token list *with no error* — so polygon/optimism (and bsc/avalanche) rendered nothing. `polygon.blockscout.com` and `explorer.optimism.io` are live and return priced `token-balances`, so both were added; bsc/avalanche have no healthy public Blockscout and stay balance-only. `BLOCKSCOUT_HOSTS` moved out to `app/api/wallet-tracker/hosts.ts` so it can be unit-tested under plain `node` (`route.logic.test.mjs`), same pattern as the stocks screener.
...
| `bsc` / `avalanche` `.blockscout.com` | 404 / 404 | Dropped from `BLOCKSCOUT_HOSTS`; those chains go straight to RPC for the balance and return no token detail. No healthy public Blockscout instance exists for them. |
