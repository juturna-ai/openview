...
// POST {reportId, emoji} — bump a reaction tally.
//
...
export async function POST(req: Request) {
  ...
  let payload: { reportId?: unknown; emoji?: unknown };
  try {
  ...
  try {
    // increment_reaction() does the upsert-and-add atomically in Postgres. A read-then-write here
    // would drop concurrent clicks (lost update) — two people reacting at once would count as one.
    const count = await rpc('increment_reaction', { p_report_id: reportId, p_emoji: emoji });
