...
const row = page.locator('.row[data-sym="NEAR-USD"]').first();
...
await page.evaluate(()=>setSymbolFlag('NEAR-USD','#ffd600'));
await page.waitForTimeout(400);
const saved = await page.evaluate(()=>localStorage.getItem('fv_flags'));
const flagStyle = await page.locator('.row[data-sym="NEAR-USD"] .flag').first().getAttribute('style');

// reload → persists
await page.reload({waitUntil:'domcontentloaded'}); await page.waitForTimeout(3800);
const flagAfterReload = await page.locator('.row[data-sym="NEAR-USD"] .flag').first().getAttribute('style');
